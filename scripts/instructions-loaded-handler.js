 ...
 * Logs instruction loading events and verifies ROSSI core rules.
 *
...
// Step 2: Verify ROSSI core rules loaded (CLAUDE.md check)
try {
  if (filePath && filePath.includes('CLAUDE.md')) {
    const fileName = path.basename(filePath);
    const dirName = path.basename(path.dirname(filePath));
    const isBkitClaudeMd = dirName === '.claude' || filePath.includes('ROSSI');
