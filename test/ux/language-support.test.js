'use strict';
/**
 * UX Tests: 9-Language Support UX (25 TC)
 * Tests detectLanguage, multi-language trigger matching for all 9 supported languages,
...
// --- UX-LANG-011b: RU 갭 분석 트리거 키워드 매칭 ---
assert('UX-LANG-011b',
  matchMultiLangPattern('это правильно? проверить', gapPatterns) === true,
  'Russian gap analysis keyword "проверить" triggers gap-detector'
);

// --- UX-LANG-012: 지원 언어 목록이 정확히 9개 ---
assert('UX-LANG-012',
  Array.isArray(SUPPORTED_LANGUAGES) && SUPPORTED_LANGUAGES.length === 9,
  'SUPPORTED_LANGUAGES contains exactly 9 languages'
);

// --- UX-LANG-013: 모든 9개 언어가 AGENT_TRIGGER_PATTERNS에 존재 ---
const allLangCovered = SUPPORTED_LANGUAGES.every(langCode =>
  AGENT_TRIGGER_PATTERNS['gap-detector'][langCode] !== undefined
);
assert('UX-LANG-013',
  allLangCovered,
  'All 9 languages are covered in gap-detector trigger patterns'
);
...
// --- UX-LANG-015b: RU pdca-iterator 트리거 키워드 매칭 ---
assert('UX-LANG-015b',
  matchMultiLangPattern('нужно улучшить этот код', iteratorPatterns) === true,
  'Russian improvement keyword "улучшить" triggers pdca-iterator'
);

// --- UX-LANG-015c: RU code-analyzer 트리거 키워드 매칭 ---
const codeAnalyzerPatternsRu = AGENT_TRIGGER_PATTERNS['code-analyzer'];
assert('UX-LANG-015c',
  matchMultiLangPattern('нужно анализировать код', codeAnalyzerPatternsRu) === true,
  'Russian analysis keyword "анализировать" triggers code-analyzer'
);

// --- UX-LANG-015d: RU detectLanguage ---
assert('UX-LANG-015d',
  detectLanguage('Привет мир') === 'ru',
  'Russian text "Привет мир" detected as "ru"'
);

// =====================================================================
// LS-001~005 (UX-LANG-016~020): 9 languages in trigger keywords
// =====================================================================

// --- LS-001: SUPPORTED_LANGUAGES includes all 9 codes ---
const expectedLangs = ['en', 'ko', 'ja', 'zh', 'es', 'fr', 'de', 'it', 'ru'];
const allPresent = expectedLangs.every(l => SUPPORTED_LANGUAGES.includes(l));
assert('LS-001',
  allPresent,
  'SUPPORTED_LANGUAGES includes EN, KO, JA, ZH, ES, FR, DE, IT, RU'
);

// --- LS-002: code-analyzer has trigger patterns for 9 languages ---
...
assert('LS-002',
  caLangs.length >= 9,
  `code-analyzer trigger patterns cover ${caLangs.length} languages (need >= 9)`
);

// --- LS-003: report-generator has trigger patterns for 9 languages ---
const reportPatterns = AGENT_TRIGGER_PATTERNS['report-generator'];
const rgLangs = reportPatterns ? Object.keys(reportPatterns) : [];
assert('LS-003',
  rgLangs.length >= 9,
  `report-generator trigger patterns cover ${rgLangs.length} languages (need >= 9)`
);

// --- LS-004: cto-lead has trigger patterns for 9 languages ---
const ctoPatterns = AGENT_TRIGGER_PATTERNS['cto-lead'];
const ctoLangs = ctoPatterns ? Object.keys(ctoPatterns) : [];
assert('LS-004',
  ctoLangs.length >= 9,
  `cto-lead trigger patterns cover ${ctoLangs.length} languages (need >= 9)`
);

// --- LS-005: starter-guide has trigger patterns for 9 languages ---
const starterPatterns = AGENT_TRIGGER_PATTERNS['starter-guide'];
const sgLangs = starterPatterns ? Object.keys(starterPatterns) : [];
assert('LS-005',
  sgLangs.length >= 9,
  `starter-guide trigger patterns cover ${sgLangs.length} languages (need >= 9)`
);
...
assert('LS-006',
  controlHasLang,
  '/control SKILL.md has EN+KO trigger keywords (v2.0.8: 9-lang via agents)'
);
...
assert('LS-007',
  auditHasLang,
  '/audit SKILL.md has EN+KO trigger keywords (v2.0.8: 9-lang via agents)'
);
...
assert('LS-008',
  rollbackHasLang,
  '/rollback SKILL.md has EN+KO trigger keywords (v2.0.8: 9-lang via agents)'
);
...
assert('LS-009',
  batchHasLang,
  '/pdca-batch SKILL.md has EN+KO trigger keywords (v2.0.8: 9-lang via agents)'
);

// --- LS-010: bkend-expert has trigger patterns for 9 languages ---
const bkendPatterns = AGENT_TRIGGER_PATTERNS['bkend-expert'];
const bkLangs = bkendPatterns ? Object.keys(bkendPatterns) : [];
assert('LS-010',
  bkLangs.length >= 9,
  `bkend-expert trigger patterns cover ${bkLangs.length} languages (need >= 9)`
);
