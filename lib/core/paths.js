...
function getBkitRoot() { return path.join(getPlatform().PROJECT_DIR, '.rossi'); }
...
const LEGACY_PATHS = {
  ...
  memory:     () => path.join(getPlatform().PROJECT_DIR, 'docs', '.rossi-memory.json'),
  snapshots:  () => path.join(getPlatform().PROJECT_DIR, 'docs', '.pdca-snapshots'),
  agentState: () => path.join(getPlatform().PROJECT_DIR, '.rossi', 'agent-state.json'),
};

const CONFIG_PATHS = {
  rossiConfig: () => path.join(getPlatform().PROJECT_DIR, 'rossi.config.json'),
...
function backupToPluginData() {
  ...
  try {
    ...
    history.push({ timestamp: new Date().toISOString(), rossiVersion: '2.0.4', backed });
