...
/**
 * rossi.config.json 로드
 * @returns {Object}
 */
function loadConfig() {
  const { globalCache } = getCache();
  const cached = globalCache.get('ROSSI-config');
  if (cached) return cached;

  const { PLUGIN_ROOT, PROJECT_DIR } = getPlatform();
  const configPaths = [
    path.join(PROJECT_DIR, 'rossi.config.json'),
    path.join(PLUGIN_ROOT, 'rossi.config.json'),
  ];
  ...
  for (const configPath of configPaths) {
    if (fs.existsSync(configPath)) {
      try {
        ...
        globalCache.set('ROSSI-config', config);
        return config;
      } catch (e) {
        // 파싱 실패 시 다음 경로 시도
      }
...
function getBkitConfig(forceRefresh = false) {
  ...
  if (!forceRefresh) {
    const cached = globalCache.get('ROSSI-full-config', 10000);
    if (cached) return cached;
  }
  ...
  const config = {
    ...
    pdca: {
      ...
      automationLevel: process.env.ROSSI_PDCA_AUTOMATION || getConfig('pdca.automationLevel', 'semi-auto'),
  ...
  globalCache.set('ROSSI-full-config', config);
  return config;
}
