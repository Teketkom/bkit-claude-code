...
function getUserConfigDir() {
  ...
  return path.join(homeDir, '.claude', 'ROSSI');
}
...
function loadContextLevel(level) {
  ...
  switch (level) {
    case 'plugin': {
      const pluginConfigPath = path.join(common.PLUGIN_ROOT, 'rossi.config.json');
    ...
    case 'project': {
      const projectConfigPath = path.join(common.PROJECT_DIR, 'rossi.config.json');
