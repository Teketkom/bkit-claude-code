...
const GUARDS = {
  ...
  guardResumeAvailable(ctx) {
    ...
    const resumePath = path.join(
      PROJECT_DIR, '.rossi', 'state', 'resume', `${ctx.feature}.resume.json`
    );
  ...
  guardCheckpointExists(ctx) {
    const { PROJECT_DIR } = getCore();
    const cpDir = path.join(PROJECT_DIR, '.rossi', 'checkpoints');
...
const ACTIONS = {
  ...
  saveResumePoint(ctx, _event) {
    ...
    const resumeDir = path.join(PROJECT_DIR, '.rossi', 'state', 'resume');
    try {
