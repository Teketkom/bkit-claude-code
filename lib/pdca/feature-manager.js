 ...
 * State stored in .rossi/state/workflows/{feature}.json.
 */
...
function _getDoLockPath() {
  ...
  return path.join(PROJECT_DIR, '.rossi', 'runtime', 'do-lock.json');
}
...
function _getFeatureWorkflowPath(feature) {
  const { PROJECT_DIR } = getCore();
  return path.join(PROJECT_DIR, '.rossi', 'state', 'workflows', `${feature}.json`);
}
