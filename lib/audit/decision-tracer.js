 ...
 * @version 2.0.0
 *
 * Records every significant decision made during ROSSI operations,
 * including alternatives considered and rationale for chosen option.
 * Storage: .rossi/decisions/YYYY-MM-DD.jsonl (one JSON object per line)
 *
 ...
 * @returns {string} Absolute path to .rossi/decisions/
 */
function getDecisionsDir() {
  return path.join(getPlatform().PROJECT_DIR, '.rossi', 'decisions');
}
