...
| 관점 | 내용 |
|------|------|
| **문제** | v2.1.83이 ~56건 변경을 포함한 대규모 릴리스로 ROSSI 영향 분석 필요 |
...
| B-83-3 | **SDK session history loss from hook messages** | **HIGH** | ROSSI 18개 hook 안정성 직접 개선 |
| B-83-4 | **Uninstalled plugin hooks continuing to fire** | **MEDIUM** | ROSSI 제거 후 hook 잔존 문제 해결 |
...
### 2.4 개선사항 — ROSSI 관련 (8건)

| ID | 변경 내용 | ROSSI 영향도 | 비고 |
|----|----------|-----------|------|
| I-83-1 | **Plugin startup — disk cache 로딩** | **HIGH** | ROSSI 36 skills, 31 agents 로딩 속도 개선 |
| I-83-2 | **`--resume` 메모리/레이턴시 개선** | MEDIUM | 대규모 세션 복원 |
| I-83-3 | Plugin MCP servers duplicate suppression | MEDIUM | ROSSI 2개 MCP 서버 충돌 방지 |
...
| D-83-2 | **`TaskOutput` deprecated → `Read`** | **NONE** | ROSSI 코드 미사용 (grep 0건) |
...
| 이슈 | 제목 | 심각도 | ROSSI 영향 |
|------|------|--------|----------|
| #38651 | Stop hook causes empty result in -p mode | **HIGH** | ROSSI Stop hook (unified-stop.js) 영향 가능 |
...
| **설명** | 사용자가 작업 디렉토리를 변경할 때 ROSSI이 자동으로 PDCA 상태를 갱신하고 프로젝트 컨텍스트를 재로딩 |
...
| **영향 파일** | `skills/enterprise/SKILL.md` 또는 `rossi-system/philosophy/` |
| **예상 공수** | 1h |
...
| **설명** | ROSSI 문서에서 Ctrl+F 참조를 Ctrl+X Ctrl+K로 업데이트 |
| **구현 범위** | 1개 문서 파일 수정 |
...
| **설명** | ROSSI unified-stop.js가 -p 모드에서 실행될 때 빈 결과 발생 가능성 모니터링 |
| **구현 범위** | 모니터링만 — ROSSI이 직접 -p 모드를 사용하지 않으므로 즉시 영향 없음 |
| **예상 공수** | monitor only |
| **YAGNI** | ⚠️ MONITOR — ROSSI CI/CD(`--bare` 또는 `-p`)에서 Stop hook 사용 시 영향 |
...
| ROSSI 구현 hook events | 18 (미변경, 신규 2개 미구현) |
