...
| 관점 | 내용 |
|------|------|
| **문제** | CC v2.1.81 릴리스의 ~26건 변경사항 중 ROSSI 영향 범위 확인 필요 |
...
| **핵심 가치** | 47번째 연속 호환 릴리스 확인 (v2.1.34~v2.1.81, zero-downtime 업그레이드 보장) + ROSSI v2.0.0 코드 변경 불필요 |
...
| 9 | Fix | Bash 명령 대시(-) 포함 문자열에 불필요한 권한 프롬프트 | **HIGH** | ✅ ROSSI Bash hooks 직접 혜택 |
| 10 | Fix | Plugin hooks: 플러그인 디렉토리 세션 중 삭제 시 프롬프트 차단 | **HIGH** | ✅ ROSSI 핵심 안정성 혜택 |
...
| 13 | Fix | `/btw`: active response 중 paste 텍스트 누락 | MEDIUM | ✅ ROSSI /btw 스킬 직접 혜택 |
...
| 16 | Fix | Invisible hook attachments가 transcript message count 부풀리기 | MEDIUM | ✅ ROSSI 18개 hook 직접 혜택 |
...
```
HIGH:   5건 (#1, #9, #10, #11, #23) — ROSSI 핵심 관련
...
| CC 변경 | 영향받는 ROSSI 컴포넌트 | 영향 유형 |
|---------|----------------------|-----------|
| --bare flag | hooks/hooks.json, scripts/* (18개 hook) | 인식 필요 — bare 모드 시 ROSSI hooks 미실행 |
...
**현황**:
- ROSSI은 interactive session 기반으로 동작 — 18개 hooks, 36개 skills 모두 interactive 세션 전제
- CI/CD 파이프라인에서 CC를 사용할 때 ROSSI hooks가 불필요하게 실행되는 케이스 존재 가능
...
3. ROSSI 문서에 "ROSSI hooks는 `--bare` 모드에서 실행되지 않음" 주의사항 명시
4. CI/CD 스크립트 예시: `claude --bare -p "run tests and report"` (ROSSI 오버헤드 제거)
...
**현황**:
- ROSSI은 GitHub repository (`rossi-dev/rossi-cto-agent-kit`)에서 설치
...
**제안**:
1. ROSSI 버전 배포 전략 재검토 — git tag/branch ref 사용 시 매 세션 자동 업데이트
...
**현황**:
- ROSSI PDCA workflow에서 Plan phase 후 context 관리가 중요
...
**제안**:
1. ROSSI 사용자에게 `showClearContextOnPlanAccept: true` 설정 권장 여부 검토
...
**YAGNI**: ⚠️ BORDERLINE — 기본값 변경이지만 ROSSI의 PreCompact hook이 이미 context 관리. P2 유지.
...
**제안 변경 없음**: preview 안정화 후 ROSSI MCP 서버에 channels 지원 추가 (P3 유지)

---

## 5. ROSSI 직접 혜택 상세 (코드 변경 불필요)

### 5.1 HIGH 영향 자동 혜택

| # | 변경사항 | ROSSI 혜택 | 관련 ROSSI 컴포넌트 |
|---|---------|-----------|-------------------|
| 9 | Bash 대시(-) 권한 프롬프트 제거 | ROSSI의 Bash PreToolUse (`scripts/unified-bash-pre.js`)에서 대시 포함 명령 실행 시 불필요한 권한 요청 감소. `npm run test-suite`, `git log --oneline` 등 일상적 명령의 UX 개선 | `scripts/unified-bash-pre.js`, `scripts/permission-request-handler.js` |
| 10 | Plugin dir 삭제 시 프롬프트 차단 방어 | ROSSI 플러그인 디렉토리가 세션 중 실수로 삭제되어도 CC가 안전하게 처리. 기존에는 전체 프롬프트 제출이 차단될 수 있었음 | `hooks/hooks.json` 전체, `scripts/*` 전체 |
...
| 8 | Node.js 18 크래시 fix | Node 18 환경 사용자의 ROSSI hooks 안정성 확보 |
| 12 | Worktree resume 전환 | worktree 기반 개발 시 세션 resume 안정성 |
| 13 | /btw paste fix | ROSSI /btw 스킬에서 active response 중 제안 입력 안정성 |
| 16 | Hook attachment transcript fix | ROSSI 18개 hooks의 invisible attachment가 transcript count에 미포함 → 정확한 대화 길이 추적 |
...
현재 v2.1.81에서 ROSSI에 영향을 미치는 신규 이슈 보고 없음. 모니터링 유지.
...
1. **CC v2.1.81 업그레이드 안전** — ROSSI v2.0.0 코드 변경 없이 업그레이드 가능
...
4. **Plugin freshness 인식** — ref-tracked plugin은 매 로드 시 re-clone됨. ROSSI 개발 중 local 변경 시 주의
...
- **Plugin freshness (#23)**: ROSSI이 ref-tracked로 설치된 경우 매 세션 시작 시 re-clone 발생. 네트워크 오프라인 시 동작 확인 필요
- **--bare flag (#1)**: CI/CD 환경에서 ROSSI hooks 의도적 비활성화 시나리오 모니터링
...
- Plan mode 기본값 변경: ROSSI PDCA Plan phase에서 context 정리 옵션이 자동 숨겨짐 → PreCompact hook이 이미 context 관리하므로 실질 영향 최소
- MCP 출력 축소: bkend MCP 5개 skills 사용 시 context 절약 효과
- Plugin freshness: ROSSI 사용자가 항상 최신 버전 사용 → 버전 불일치 문제 감소
...
**ROSSI 관점**:
- `--bare` 모드에서 ROSSI은 완전히 비활성화됨 (hooks, skills 모두 스킵)
- CI/CD, 스크립팅 용도에 적합 — ROSSI 오버헤드 없이 순수 LLM 호출
- ROSSI PDCA 워크플로우와는 독립적 사용 케이스
...
**ROSSI 관점**:
- `plugin.json`에 `"repository": "https://github.com/rossi-dev/rossi-cto-agent-kit"` 명시
- main branch tracking 시 → 매 세션 시작마다 최신 main 자동 반영
- **장점**: 사용자가 항상 최신 ROSSI 사용, 수동 업데이트 불필요
