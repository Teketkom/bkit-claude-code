...
| 관점 | 내용 |
|------|------|
| **문제** | CC v2.1.80 릴리스의 ~19건 변경사항 중 ROSSI 영향 범위 확인 필요 |
...
| **핵심 가치** | 46번째 연속 호환 릴리스 확인 (v2.1.34~v2.1.80, zero-downtime 업그레이드 보장) + ROSSI v2.0.0 코드 변경 불필요 |
...
| 3 | Feature | CLI tool usage detection for plugin tips | LOW | ✅ ROSSI 자동 혜택 (더 정확한 추천) |
...
| 11 | Improve | `@` file autocomplete 대용량 레포 응답성 개선 | MEDIUM | ✅ 자동 혜택 (ROSSI 40K LOC) |
...
| 15 | Improve | Plugin install tips 단순화 (1-step /plugin install) | MEDIUM | ✅ ROSSI 설치 UX 직접 개선 |
...
| 시작 메모리 80MB 절감 | 전체 ROSSI 세션 | 자동 혜택 |
| Memory stale check | auto-memory 시스템 | 자동 혜택 |
| Plugin install 단순화 | ROSSI 설치 경험 | 자동 혜택 |
...
**현황**:
- ROSSI agents 31개: 이미 effort 설정 완료 (ENH-120, v2.1.78)
- ROSSI skills 36개: effort 미설정

**제안**: 36개 스킬 SKILL.md에 effort frontmatter 추가
- report-generator, rossi-templates → `effort: low`
...
**현황**: ROSSI은 statusline scripts를 현재 사용하지 않음 (CLI dashboard는 별도 구현)

**제안**: ROSSI CLI dashboard에 rate limit 정보 표시 고려
...
**현황**: ROSSI은 marketplace git repository 방식으로 배포
...
**현황**: ROSSI MCP 서버 2개 (rossi-pdca, rossi-analysis) 운용 중

**제안**: research preview 안정화 후 ROSSI MCP 서버에 channels 지원 추가
- PDCA 상태 변경 시 자동 알림
- 백그라운드 분석 완료 push

**영향 파일**: `mcp-servers/rossi-pdca/`, `mcp-servers/rossi-analysis/`
...
| #36747 | LOW | ❌ | MCP host fastmcp 3.1.1 호환 문제 — ROSSI MCP는 자체 구현 |
| #36741 | LOW | ❌ | commit-push-pr skill allowed-tools 누락 |
| #36740 | LOW | ⚠️ | 스킬 description 미표시 — ROSSI 스킬 노출에 영향 가능 |
...
1. **CC v2.1.80 업그레이드 안전** — ROSSI v2.0.0 코드 변경 없이 업그레이드 가능
...
- **#36740** (스킬 description 미표시): ROSSI 스킬 노출에 영향 가능 — 재현 확인 필요
