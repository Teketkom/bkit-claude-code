...
| 관점 | 내용 |
|------|------|
| **문제** | `CLAUDE_PLUGIN_DATA` 백업이 플러그인 전역 경로에 저장되어, 새 프로젝트에 ROSSI 설치 시 다른 프로젝트의 PDCA 상태(23KB+)가 무검증 복원됨. v1.6.2 ENH-119 회귀 버그 |
| **해결** | `backupToPluginData()`에 `meta.json` 프로젝트 식별자 저장 + `restoreFromPluginData()`에 프로젝트 일치 검증 가드 5단계 + `globalCache` 프로젝트 네임스페이스 |
| **기능/UX 효과** | 멀티 프로젝트 환경에서 ROSSI 설치/전환 시 각 프로젝트가 항상 독립된 깨끗한 PDCA 상태로 시작. "내 프로젝트에는 내 데이터만" 원칙 확립 |
