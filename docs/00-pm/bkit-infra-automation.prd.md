# PRD: ROSSI Infrastructure Automation (Self-Healing CI/CD)

**Feature**: rossi-infra-automation
**Version**: 2.0
**Date**: 2026-03-24
...
| 관점 | 내용 |
|------|------|
| **Problem** | ROSSI은 PDCA 워크플로우를 통해 코드 생성까지 자동화하지만, 배포 이후의 운영 자동화(CI/CD, 모니터링, 장애 대응)는 수동으로 남아있다. phase-9-deployment 스킬과 infra-architect 에이전트가 존재하나, 실제 파이프라인 통합이 없어 "코드 작성 -> 프로덕션 안정화"까지의 루프가 닫히지 않는다. |
| **Solution** | 6-Layer Self-Healing CI/CD Architecture(CI/CD + Infrastructure + Security + Observability + Error Tracking + Self-Healing)를 ROSSI 플러그인에 통합한다. 핵심은 **Living Context 시스템**(Scenario Matrix, Invariants Registry, Impact Map, Incident Memory)으로 Self-Healing 수정 품질을 보장하고, 6개 Human Intervention Point를 보존하면서 자동화를 최대화한다. |
...
| **Core Value** | ROSSI을 "코드 생성 도구"에서 "컨텍스트 인지형 풀스택 DevOps 자동화 플랫폼"으로 진화. AI가 "왜 이렇게 만들었는지" 이해하고 수정하는 Self-Healing. |
...
```
[Desired Outcome]
ROSSI 사용자가 코드 작성부터 프로덕션 안정 운영까지
...
│   ├── 프로덕션 메트릭이 ROSSI Check 단계에 반영 안됨
│   ├── Grafana/CloudWatch 데이터 활용 불가
│   └── [Solutions]
│       ├── S4: monitoring-bridge MCP 서버 (Prometheus/CloudWatch -> ROSSI)
│       ├── S5: PDCA Check에 운영 메트릭 자동 포함
│       └── S6: AlertManager -> ROSSI Hook 연동
│
...
| A4 | ROSSI 사용자가 AWS + K8s Enterprise 인프라를 사용한다 | 7 | 7 | 49 | 사용자 설문 + 프로젝트 레벨 분석 |
...
| **6. Our solution** | ROSSI의 PDCA 워크플로우에 5-Layer Self-Healing CI/CD를 통합하여, `/pdca deploy` -> 모니터링 자동 연동 -> Self-Healing 자동 수정 PR -> Human Review 게이트까지 일관된 AI Native 경험을 제공한다 |
...
| **Customer Segment** | Enterprise 레벨 ROSSI 사용자, DevOps 엔지니어, AI Native 개발팀, 스타트업 CTO |
| **Unique Value Proposition** | "코드 작성부터 프로덕션 자가 치유까지, ROSSI 하나로 완성하는 AI Native DevOps" |
| **Solution** | 5-Layer Self-Healing CI/CD를 ROSSI 플러그인으로 통합 (배포 스킬 + 모니터링 MCP + Self-Healing 에이전트) |
| **Channels** | ROSSI marketplace, Claude Code 플러그인 생태계, DevOps 커뮤니티 (Hashnode, Dev.to), 기술 블로그 |
| **Revenue Streams** | ROSSI Enterprise 라이선스 (Self-Healing 포함), 관리형 모니터링 대시보드 구독 |
| **Cost Structure** | Claude API 비용 (Self-Healing 분석), AWS 인프라 비용, 개발/유지보수 인력 |
| **Key Metrics** | MTTR 감소율, Self-Healing PR 승인율, 배포 빈도, 배포 실패율 |
| **Unfair Advantage** | ROSSI의 기존 PDCA 워크플로우 + 37개 에이전트 + Hook 시스템과의 네이티브 통합. 경쟁사는 CI/CD OR AI 코딩 중 하나만 제공 |
...
| **ST (강점-위협)** | PDCA 워크플로우 통합이라는 차별점을 강화. GitHub Copilot은 코드 생성에 집중하지만, ROSSI은 Plan->Deploy->Monitor->Heal 전체 사이클을 커버 |
| **WO (약점-기회)** | monitoring-bridge MCP 서버를 새로 개발하여 Prometheus/CloudWatch 데이터를 ROSSI으로 유입. Automation Level을 L6까지 확장하여 Self-Healing 레벨 정의 |
...
| **구매자 교섭력** | 중 | Enterprise 고객은 대안이 많지만, PDCA 통합 경험은 ROSSI만 제공 |
...
| **ROSSI 사용 수준** | Enterprise 레벨, 현재 PDCA Plan/Design/Do만 사용 |
...
| **JTBD** | "DevOps를 잘 몰라도, ROSSI이 CI/CD 파이프라인을 자동으로 만들어주고 관리해줬으면 좋겠다" |
| **기대 가치** | 배포 설정 시간 90% 절약, DevOps 학습 곡선 제거 |
| **ROSSI 사용 수준** | Dynamic 레벨, 현재 Vercel 수동 배포 |
...
| **ROSSI 사용 수준** | Enterprise 레벨, 조직 내 파일럿 검토 중 |
...
│                              │                             │                           │                          │
│ "ROSSI으로 코드는             │ "Self-Healing이              │ "DEV 환경에서              │ "프로덕션에서               │ "Self-Healing PR이
...
| # | 경쟁사 | 유형 | 강점 | 약점 | ROSSI 차별점 |
|---|--------|------|------|------|-------------|
| 1 | **GitHub Copilot Workspace** | AI 코딩 + CI | GitHub 네이티브, 대규모 사용자 | DevOps 자동화 없음, Self-Healing 없음, PDCA 없음 | ROSSI은 Plan->Heal 전체 사이클 커버 |
| 2 | **GitLab Auto DevOps** | CI/CD 자동화 | 빌트인 CI/CD, Auto Deploy, SAST/DAST | AI 코드 수정 없음, Self-Healing 없음 | ROSSI은 AI 기반 에러 분석+수정 제공 |
| 3 | **Gitar.ai** | Self-Healing CI/CD | AI 기반 파이프라인 자동 수정 | 코드 생성/PDCA 없음, 독립 도구 | ROSSI은 개발 전체 워크플로우와 통합 |
| 4 | **Qodo (CodeiumAI)** | AI 코드 리뷰 | CI/CD 파이프라인 내 AI 리뷰 | 인프라 자동화 없음, 배포 미지원 | ROSSI은 인프라+코드+운영 통합 |
| 5 | **Harness AI** | CI/CD + AIOps | ML 기반 배포 최적화, 롤백 자동화 | 고가, 학습 곡선 높음, Claude Code 미지원 | ROSSI은 Claude Code 네이티브, 오픈소스 |
...
| **SOM** | $18M | Claude Code 사용 팀 3만 (1%) x 연 $600 ROSSI 구독 |
...
| 기준 | 점수 (1-5) | 설명 |
|------|-----------|------|
| **접근성** | 5 | 기존 ROSSI Enterprise 사용자 - 이미 채널 확보 |
| **Pain 강도** | 4 | 배포+모니터링 수동 작업에 대한 불만 높음 |
| **구매력** | 3 | 스타트업은 예산 제한적, 중견기업은 충분 |
| **전략적 가치** | 5 | Enterprise 레퍼런스 확보 -> 시장 확대의 기반 |
| **총점** | **17/20** | |

**Beachhead**: ROSSI Enterprise 레벨을 이미 사용 중인 10-50명 규모 스타트업 개발팀, AWS + K8s 인프라 운영 중, 전담 DevOps 엔지니어 1-3명.

### 4.3 GTM Strategy

| 채널 | 전략 | 기대 효과 |
|------|------|-----------|
| **ROSSI Marketplace** | Self-Healing CI/CD를 Enterprise 전용 기능으로 출시 | 기존 사용자 업그레이드 유도 |
| **Claude Code Plugin 생태계** | claude-code-action과의 공식 통합 예제 공개 | Claude Code 사용자 유입 |
| **기술 블로그/YouTube** | "ROSSI으로 Self-Healing CI/CD 구축하기" 시리즈 | SEO + 커뮤니티 인지도 |
...
| 질문 | ROSSI 답변 |
|------|-----------|
| "GitHub Copilot이면 충분하지 않나요?" | Copilot은 코드 생성에 집중합니다. ROSSI은 코드 생성 + 배포 + 모니터링 + Self-Healing까지 전체 PDCA 사이클을 커버합니다. |
| "GitLab Auto DevOps와 뭐가 다른가요?" | GitLab은 CI/CD 자동화에 강하지만, AI 기반 에러 분석과 자동 코드 수정은 없습니다. ROSSI은 Claude Code를 활용한 Self-Healing을 제공합니다. |
| "Harness AI가 더 성숙하지 않나요?" | Harness는 엔터프라이즈 CI/CD에 강하지만, 연 $50K+ 비용과 높은 학습 곡선이 있습니다. ROSSI은 Claude Code 생태계에 네이티브하고, 개발 워크플로우와 통합됩니다. |
| "Self-Healing이 프로덕션을 망가뜨리면?" | ROSSI은 6개 Human Intervention Point를 엄격히 유지합니다. Self-Healing PR은 반드시 사람이 리뷰/승인해야 머지되고, ArgoCD가 자동 배포합니다. |

---

## 5. ROSSI Integration Analysis (핵심 분석)
...
| `monitoring-bridge` MCP 서버 | MCP Server | Prometheus/CloudWatch -> ROSSI 메트릭 브릿지 | 높음 |
...
| `rossi.config.json` | automation.defaultLevel: 2 | L5/L6 설정 추가, self-healing 설정 섹션 추가 |
...
#### 5.1.3 6-Layer -> ROSSI 매핑 (v2.0 업데이트)

| Architecture Layer | ROSSI 매핑 | 구현 방식 |
|-------------------|-----------|-----------|
...
| **L5: Error Tracking** | `heal-trigger` Hook + Slack 연동 | Sentry webhook -> Slack(P1~P4) -> ROSSI 이벤트 트리거 -> PagerDuty 에스컬레이션 |
| **L6: Self-Healing** | `self-healing` 에이전트 + **Living Context** | Claude Code가 **컨텍스트 로딩 후** 수정 → ROSSI /pdca-iterate(max 5) → 100% Test Pass Gate → Auto PR → **Escalation/Audit/Rollback** |
...
| **Invariants Registry** | `.rossi/invariants.yaml` | 사람이 **핵심 비즈니스 룰만 작성** (10~20개) | critical invariant 위반 시 수정 거부 → 사람 에스컬레이션 |
| **Impact Map** | `.rossi/impact-map.yaml` | code-analyzer가 **정적 분석으로 자동 생성** | 수정 파일의 blast radius 자동 산정, 영향받는 파일 연쇄 검증 |
| **Incident Memory** | `.rossi/incident-memory.yaml` | Self-Healing 실행 시 **자동 누적** | 과거 동일 영역 장애 기록 참조, anti-pattern 반복 방지 |
...
| 7 | **ROSSI 시장 포지션** | AI 코딩 도구 (코드 생성 중심) | AI Native DevOps 플랫폼 (생성 + 운영 통합) | TAM 3x 확대 |

### 5.3 통합 시 나빠지는 점 (구체적)

| # | 악화 사항 | 상세 설명 | 심각도 | 완화 전략 |
|---|-----------|-----------|--------|-----------|
| 1 | **복잡성 급증** | 현재 37개 에이전트 + 28개 스킬에 Self-Healing 관련 컴포넌트 7개+ 추가. 플러그인 크기와 의존성 대폭 증가 | 높음 | 모듈화: Self-Healing을 별도 패키지(`rossi-infra`)로 분리, 선택적 설치 |
...
| F-05 | `monitoring-bridge` MCP 서버 | P1 | 높음 | Prometheus/CloudWatch 메트릭을 ROSSI으로 브릿지 |
...
| F-10 | Automation Level L5/L6 | P1 | 낮음 | rossi.config.json에 L5(Deploy Auto), L6(Self-Heal) 정의 |
...
| F-17 | **Invariants Registry** | P1 | 낮음 | `.rossi/invariants.yaml` — 핵심 비즈니스 룰 불변조건 관리, critical 위반 시 수정 거부 |
...
- F-17: **Invariants Registry** (`.rossi/invariants.yaml`)
...
| 2 | **Claude API 비용이 예상보다 5-10x 초과** | 높음 | 높음 | Rate limiting, 에러 중복 제거, 분석 결과 캐싱, 비용 상한선 설정 (rossi.config.json) |
...
| US-03 | 풀스택 개발자 | ROSSI이 내 프로젝트에 맞는 CI/CD 파이프라인을 자동 생성 | DevOps 지식 없이도 안정적인 배포가 가능하다 | P0 |
...
| TS-07 | 감사 로그 기록 | Self-Healing PR 생성 후 | `.rossi/heal-audit.json`에 에러 내용, 분석 결과, PR URL, 타임스탬프 기록 | US-04 |
...
|-------------|------|--------|--------|
| ROSSI 사용자 (Enterprise) | 최종 사용자 | 기능 품질, 배포 안정성, 학습 곡선 | 높음 |
| ROSSI 사용자 (Dynamic) | 잠재 업그레이드 사용자 | 접근성, 가격, Docker 지원 여부 | 중간 |
| ROSSI Core Team | 개발팀 | 구현 복잡도, 유지보수성, 기존 아키텍처 영향 | 높음 |
...
이 PRD는 ROSSI PM Agent Team이 다음 프레임워크를 사용하여 생성했습니다:
